...
**Архив в монорепо:** неактуальные и завершённые копии лежат в **`Projects/Non actual/`** (например `TradeTherapyBot`, `PastuhiBot`, `Damir`, `Limits`, `xxx`, `HH`, `Chess`, …). Активный отдельный репозиторий **VPN** — в **`Projects/VPN/`** (не в архиве).
...
4. `nikkronos/Cursor_Projects` — репозиторий на GitHub для всех проектов скопом (кроме env_vars); локально папка обычно **`Cursor_Projects`**. Раньше репозиторий назывался `Cursor_Projectcs` (редирект на GitHub).

**Локальный путь:** `C:\Users\krono\OneDrive\Рабочий стол\Cursor_Projects`

**Важно:** Основного репозитория нет. При создании нового проекта агент должен спросить: создать новый репозиторий или коммитить в существующий (например, `nikkronos/Cursor_Projects`)?
...
**Расположение:**
- Локально: `Projects/Non actual/TradeTherapyBot/`
- На сервере: `/opt/tradetherapybot` (предположительно)

**Документы:**
- `Projects/Non actual/TradeTherapyBot/ROADMAP_TRADETHERAPYBOT.md` - планы развития
- `Projects/Non actual/TradeTherapyBot/DONE_LIST_TRADETHERAPYBOT.md` - выполненные задачи
- `Projects/Non actual/TradeTherapyBot/SESSION_SUMMARY_YYYY-MM-DD.md` - последняя сессия
- `Projects/Non actual/TradeTherapyBot/docs/` - база знаний проекта
...
**Статус:** ⏸️ Архив в монорепо (`Projects/Non actual/PastuhiBot/`); прод по-прежнему на сервере  
**Описание:** Telegram-бот для копирования содержимого определенных Telegram каналов (бот-перелив).

**Особенности:**
- Копия в **`Projects/Non actual/PastuhiBot/`** коммитится в **этот** монорепозиторий как снимок; деплой на сервер — отдельно, не через обязательный push монорепо
- **Прод с 2026-04-10:** боевые процессы (внутренние имена **`hamster26_*`**, **`hamster93_*`**) перенесены на **Fornex** из‑за недоступности Telegram API с Timeweb; см. `Projects/Non actual/PastuhiBot/docs/SERVER_MIGRATION_FORNEX_2026-04-10.md` и кратко для разработчика: `Projects/Non actual/PastuhiBot/DEV_HANDOFF_FORNEX_2026-04-10.md`
- Создан с разработчиком (копия с сервера)
- Изменения применяются вручную после тестирования

**Расположение:**
- Локально: `Projects/Non actual/PastuhiBot/`
...
**Документы:**
- `Projects/Non actual/PastuhiBot/ROADMAP_PASTUHIBOT.md` - планы развития
- `Projects/Non actual/PastuhiBot/DONE_LIST_PASTUHIBOT.md` - выполненные задачи
- `Projects/Non actual/PastuhiBot/SESSION_SUMMARY_YYYY-MM-DD.md` - последняя сессия
- `Projects/Non actual/PastuhiBot/docs/` - база знаний проекта
- `Projects/Non actual/PastuhiBot/docs/SERVER_MIGRATION_FORNEX_2026-04-10.md` — миграция на Fornex (технически)
- `Projects/Non actual/PastuhiBot/DEV_HANDOFF_FORNEX_2026-04-10.md` — передача разработчику
...
**Важно:** Пути в документации ведут в архив **`Projects/Non actual/PastuhiBot/`**.

---

### 3. ParentChildResearch

**Статус:** Архив в монорепо (`Projects/Non actual/ParentChildResearch/`)  
**Описание:** Исследовательский проект для диплома.

**Особенности:**
- Снимок хранится в **`Projects/Non actual/ParentChildResearch/`**
- Исследовательская работа
- Может быть удалён после завершения

**Расположение:**
- Локально: `Projects/Non actual/ParentChildResearch/`

**Документы:**
- `PROJECT_SUMMARY.md` - описание проекта
- `docs/` - документация исследования

**Важно:** Снимок в **`Projects/Non actual/ParentChildResearch/`**; при необходимости можно удалить из архива.

---

### 4. TestN8N

**Статус:** ❌ Не в монорепо (каталог в **корне** репозитория, в `.gitignore`)  
**Описание:** Тестовый Telegram-бот для проверки интеграции с N8N.

**Особенности:**
- **Исключён из Git** через `.gitignore` — локальные эксперименты
- Используется для быстрого тестирования N8N интеграции
- Простой бот с базовыми командами
- Отправляет события в N8N webhook

**Расположение:**
- Локально: `TestN8N/` (корень `Cursor_Projects`, не внутри `Projects/`)
...
**Особенности:**
- **Отдельный Git-репозиторий** — не входит в TradeTherapyBot; коммиты и пуш из папки `Projects/Non actual/xxx/` идут в **nikkronos/kopiya-iksuyemsya**
...
**Расположение:**
- Локально: папка `Projects/Non actual/xxx/` (свой `.git`, свой remote)
- На сервере (прод): **Fornex** — `/opt/kopiya-iksuyemsya`, venv, `systemctl enable --now kopiya-iksuyemsya.service`

**Документы:**
- `Projects/Non actual/xxx/docs/ДВА_РЕПОЗИТОРИЯ_И_ДЕПЛОЙ.md` — пошаговое разделение репозиториев и деплой
- `Main_docs/TELEGRAM_MIGRATION_TIMWEB_FORNEX_2026-04-10.md` — инцидент блокировки Telegram с Timeweb и миграция на Fornex
- В папке `Projects/Non actual/xxx/`: README, ROADMAP, DONE_LIST, TESTING_GUIDE и т.д.
...
**Важно:** Коммиты для этого проекта делаются **в папке `Projects/Non actual/xxx/`** (отдельный `.git`); пуш — в репозиторий **kopiya-iksuyemsya**, не в TradeTherapyBot.
...
**Расположение:**
- Локально (архив в монорепо): `Projects/Non actual/Damir/`
- На сервере: `/opt/futures_auction`

**Документы:**
- `Projects/Non actual/Damir/TRANSFER_OTHER_PERSON.md` — инструкция для передачи человеку / агенту (есть также `.html`, `.pdf`)
- `Projects/Non actual/Damir/ROADMAP_DAMIR.md` — планы
- `Projects/Non actual/Damir/DONE_LIST_DAMIR.md` — выполненные задачи
- `Projects/Non actual/Damir/SESSION_SUMMARY_YYYY-MM-DD.md` — последняя сессия
- `Projects/Non actual/Damir/docs/` — база знаний, спеки
...
**Важно:** Код также в репозитории **Futures_auction**; снимок в монорепо — для истории и агентов.
...
**Расположение:**
- Локально: `Projects/Non actual/Limits/`
...
├── Projects/
│   ├── VPN/                   # ✅ Отдельный репо nikkronos/vpnservice (в .gitignore монорепо)
│   └── Non actual/            # Архив: TradeTherapyBot, PastuhiBot, Damir, xxx, Limits, HH, Chess, …
├── TestN8N/                   # ❌ Локально, в .gitignore
├── docs/                      # Общая документация (часть дублей убрана — см. архив бота)
└── .gitignore
```
...
1. **Монорепо `Cursor_Projects`** — архив **`Projects/Non actual/`** коммитится сюда; отдельные remotes см. ниже.
2. **TradeTherapyBot** — отдельный репо `nikkronos/TradeTherapyBot`; локальная копия в **`Projects/Non actual/TradeTherapyBot/`** (история/снимок).
3. **Копия иксуюемся** — репо `nikkronos/kopiya-iksuyemsya`; рабочая папка **`Projects/Non actual/xxx/`** (свой `.git`).
4. **VPN** — репо `nikkronos/vpnservice`; папка **`Projects/VPN/`** (в монорепо не трекается).
5. **TestN8N** — каталог в корне, в `.gitignore`, только локально.
