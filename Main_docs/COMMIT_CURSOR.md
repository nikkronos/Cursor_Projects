# Правила коммитов для Cursor

## Репозиторий Git в корне `Cursor_Projects`

- **`origin`** — **`https://github.com/nikkronos/Cursor_Projects.git`**: общий монорепозиторий на GitHub (`Main_docs/`, `Projects/`, и т.д.). Имя репозитория на сайте — **`Cursor_Projects`** (совпадает с типичным именем локальной папки). Старый URL `…/Cursor_Projectcs.git` GitHub перенаправляет сюда; для `git remote set-url` и новых клонов используй **`Cursor_Projects`**. Команды в корне: `git push origin main` отправляют сюда.
- **`nikkronos/TradeTherapyBot`** — **отдельный** репозиторий кода бота. В корневом клоне `Cursor_Projects` он подключён как дополнительный remote **`tradetherapybot`**, а не как `origin`. Явно: `git fetch tradetherapybot`, `git push tradetherapybot …` — только если нужно взаимодействовать с репозиторием TradeTherapyBot; **не подменяй им `origin`**, иначе пуш уйдёт не в тот репозиторий.
- Если **`git fetch origin`** выдаёт **`Repository not found`**: проверь URL (`…/Cursor_Projects.git`), приватность репозитория и вход (`gh auth login`, PAT в Windows) или SSH (`git@github.com:nikkronos/Cursor_Projects.git`).

### GitHub: заново залогиниться после отзыва токенов

Если **`Invalid username or token` / `Password authentication is not supported`**: старый PAT в Windows больше не действует. Сделай одно из:

1. **`gh auth login`** (рекомендуется) и снова `git push origin main`.
2. Либо **Параметры Windows → Учётные записи → Диспетчер учётных данных** — удали сохранённые пароли для `git:https://github.com`, затем при следующем `git push` введи **новый** fine-grained/classic PAT с правом `Contents: Read and write` для репо `Cursor_Projects`.

### Push protection (GH013): «Push cannot contain secrets»

GitHub сканирует **все объекты**, которые ты отправляешь веткой (включая старую историю). Сообщение указывает **коммит и путь файла** — там когда-то был PAT (`ghp_…` / `github_pat_…`).

1. **Токен считать скомпрометированным** — отозвать в настройках GitHub (ты уже удалил токены — хорошо).
2. **Убрать секрет из истории**, затем снова `git push`:
   - Установить [git-filter-repo](https://github.com/newren/git-filter-repo) (`pip install git-filter-repo`), сделать резервную копию папки `.git`, затем, например, удалить проблемный файл из всей истории:  
     `git filter-repo --path ПУТЬ/К/ФАЙЛУ.md --invert-paths --force`  
     либо замена текста по [документации replace-text](https://htmlpreview.github.io/?https://github.com/newren/git-filter-repo/blob/docs/html/git-filter-repo.html#_replacing_text_in_files).
   - Если история не нужна: **новая ветка без родителя** (`git checkout --orphan …`, один коммит со снимком файлов) и **`git push --force`** (осознанно теряется вся старая история на `main`).
3. Кнопка **Allow secret** в URL из лога GitHub — только если уверен, что это ложное срабатывание; для настоящего PAT **не использовать**.

Если в **локальном** `git log` нет коммита из сообщения GitHub, секрет мог быть только в другой копии репозитория или уже в удалённой истории — ориентируйся на путь и паттерн из письма/лога GitHub и повтори `filter-repo` на той копии, из которой делается push.

## Логика коммитов

### Что коммитится в репозиторий TradeTherapyBot (nikkronos/TradeTherapyBot):

✅ **В отдельном клоне TradeTherapyBot** — только код бота (корень того репозитория: `main.py`, `handlers/`, `loader.py`, `docs/` и т.д.).

✅ **В монорепозитории `Cursor_Projects`** — архивная копия кода бота в **`Projects/Non actual/TradeTherapyBot/`** (не дублировать второй полной копией вне архива). Общие правила и быстрый старт агента — в **`Main_docs/`**.

❌ **Не смешивать с репозиторием `nikkronos/TradeTherapyBot` при пуше в него:**
- Архив монорепо **`Projects/Non actual/`** — это **другой** Git (`origin` → `Cursor_Projects`). Пуш в `TradeTherapyBot` делай из отдельного клона или из **`Projects/Non actual/TradeTherapyBot/`**, если там настроен remote на `TradeTherapyBot`.
- **«Копия иксуюемся»** — свой репозиторий **nikkronos/kopiya-iksuyemsya**; рабочая копия в **`Projects/Non actual/xxx/`** (свой `.git`). См. `docs/ДВА_РЕПОЗИТОРИЯ_И_ДЕПЛОЙ.md`.

### Настройка .gitignore (монорепо)

Исключения в корне `Cursor_Projects` описаны в **`.gitignore`** (например **`Projects/VPN/`**, **`TestN8N/`**). Архив **`Projects/Non actual/`** в монорепо **трекается** и коммитится в **`nikkronos/Cursor_Projects`**.

## Процесс коммита

### Автоматический коммит (предпочтительно)

Агент может делать коммиты автоматически после:
1. ✅ Все изменения протестированы
2. ✅ Проверены связанные функции
3. ✅ Нет ошибок линтера
4. ✅ Документация обновлена
5. ✅ Временные файлы удалены

**Команды, которые выполняет агент:**
```bash
# 1. Проверить статус
git status

# 2. Добавить все изменения (VPN и TestN8N по .gitignore исключатся)
git add .

# 3. Сделать коммит с описанием
git commit -m "тип: краткое описание

- Детали изменений
- Что было исправлено
- Что было добавлено"

# 4. Отправить в репозиторий
git push origin main
```

### Ручной коммит (если нужно)

Если агент не может выполнить команды автоматически (проблемы с кодировкой, доступом и т.д.), он предоставляет пользователю команды для выполнения вручную.

## Типы коммитов

Используй стандартные типы коммитов:

- `feat:` - новая функциональность
- `fix:` - исправление бага
- `docs:` - изменения в документации
- `refactor:` - рефакторинг кода
- `test:` - добавление/изменение тестов
- `chore:` - обновление зависимостей, настройка и т.д.

**Примеры:**
```
feat: добавлена система массовых уведомлений
fix: исправлены SQL-инъекции в add_channel.py
docs: унифицирована структура документов
refactor: улучшена структура базы данных
```

## Важные правила

1. **Всегда проверяй .gitignore** — что **`Projects/VPN/`** и **`TestN8N/`** не попадут в коммит монорепо без необходимости
2. **Проверяй статус перед коммитом** - `git status` покажет, что будет добавлено
3. **Пиши понятные сообщения коммитов** - описание должно быть ясным
4. **Коммить только после тестирования** - не коммить сломанный код
5. **Обновляй документацию** - если изменилась структура, обнови соответствующие файлы

## Структура монорепозитория Cursor_Projects (корень папки)

```
Cursor_Projects/                # Корень локально; remote origin → github.com/nikkronos/Cursor_Projects
├── .gitignore
├── Main_docs/                   # Общие правила, COMMIT_CURSOR.md, …
├── Projects/
│   ├── VPN/                     # Отдельный репо vpnservice (в .gitignore монорепо)
│   └── Non actual/              # Архив неактуальных проектов (в монорепо коммитится)
├── docs/                        # Общая документация
└── TestN8N/                     # Локально (в .gitignore)
```

Отдельный клон **TradeTherapyBot** (`nikkronos/TradeTherapyBot`) — свой каталог и свой `origin`; в монорепо `Cursor_Projects` на него смотрит remote **`tradetherapybot`** (см. выше).

Коммиты для **Копия иксуюемся**: выполняются в папке **`Projects/Non actual/xxx/`**, пуш в **nikkronos/kopiya-iksuyemsya**. Подробно: `docs/ДВА_РЕПОЗИТОРИЯ_И_ДЕПЛОЙ.md`.

## CI/CD

Если настроен CI/CD (GitHub Actions):
- После `git push` автоматически запускаются тесты
- При успешных тестах → автоматический деплой на сервер
- Проверяй статус в GitHub Actions

## Частые проблемы

### Проблема: PowerShell не может выполнить команды из-за русских символов в пути

**Решение:** Агент предоставляет команды для ручного выполнения или использует альтернативные методы.

### Проблема: Файлы из ParentChildResearch или PastuhiBot попали в коммит

**Решение:** 
1. Проверь `.gitignore` - должны быть исключения
2. Если файлы уже добавлены: `git reset HEAD <файл>` и добавь в `.gitignore`
3. Если файлы уже закоммичены: `git rm --cached -r <папка>` и обнови `.gitignore`

---

**Последнее обновление:** 11.04.2026

























