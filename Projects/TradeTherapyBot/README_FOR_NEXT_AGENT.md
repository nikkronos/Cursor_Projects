...
> **⏸️ Проект приостановлен (неактуален) с 2026-02-21.** Бот выключен на сервере Timeweb (`systemctl stop` + `disable tradetherapybot.service`). Репозиторий и документация актуальны; для повторного запуска — `systemctl enable` + `start tradetherapybot.service` на сервере.
