...
Проект живёт в **отдельном репозитории** (например nikkronos/kopiya-iksuyemsya). Полная пошаговая инструкция по двум репозиториям и деплою — в корне монорепо: `docs/ДВА_РЕПОЗИТОРИЯ_И_ДЕПЛОЙ.md`.

### systemd (кратко)

1. Клонировать репозиторий на сервер, например: `/opt/kopiya-iksuyemsya`.
2. Скопировать `kopiya-iksuyemsya.service` в `/etc/systemd/system/`, поправить пути (`WorkingDirectory`, `ExecStart` = `/opt/kopiya-iksuyemsya`).
3. Создать `env_vars.txt` в `/opt/kopiya-iksuyemsya` (секреты не коммитятся).
4. `sudo systemctl daemon-reload && sudo systemctl enable kopiya-iksuyemsya.service && sudo systemctl start kopiya-iksuyemsya.service`
5. Логи: `sudo journalctl -u kopiya-iksuyemsya.service -f`

### CI/CD (GitHub Actions)

В репозитории kopiya-iksuyemsya в **Secrets** задать `SSH_*` и `DEPLOY_PATH=/opt/kopiya-iksuyemsya`. При пуше в `main` workflow обновит код и перезапустит `kopiya-iksuyemsya.service`.
